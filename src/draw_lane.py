...
def lr_curvature(binary_warped):
  ...
  for window in range(nwindows):
      ...
      if len(good_left_inds) > minpix:
          leftx_current = int(dacay_rate * leftx_current + (1-dacay_rate) * np.int(np.mean(nonzerox[good_left_inds])))
      if len(good_right_inds) > minpix:        
          rightx_current = int(dacay_rate * rightx_current + (1-dacay_rate) * np.int(np.mean(nonzerox[good_right_inds])))
